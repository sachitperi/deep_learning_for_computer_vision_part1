...
for (i, imagePath) in enumerate(imagePaths):
    ...
    try:
        # load the image and convert it to grayscale, then pad the image to ensure that digits caught on the border of the image are retained
        ...
        # find the contours of the image, keeping only the four largest ones
        ...
        for c in cnts:
            ...
            # grab the key that was construct the path the output directory
            key = chr(key).upper()
            dirPath = os.path.sep.join(args["annot"], key)

            # if the output directory doesnot exist, create it
    ...
    # we are trying to control-c, so break from the loop (you still need to press a key for the active window to trigger this)
